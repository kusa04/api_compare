...
- 各リクエストの所要時間を個別に記録し、**経過時間(全体)・平均(1件あたり)・最小・最大** を算出
  - **経過時間(全体)**: 全リクエストが完了するまでの wall clock 時間（スループット指標）
  - **平均(1件あたり)**: 個別リクエスト所要時間の平均（レイテンシ指標）
...
`go run benchmark/main.go -n 1000 -c 50` の実行結果の一例:

### テスト1: シングルリクエスト (1ユーザー取得)

|  | gRPC | REST |
|---|---|---|
| 経過時間(全体) | 58.96ms | 34.72ms |
| 平均(1件あたり) | 58.9µs | 34.7µs |
| 最小 | 30.9µs | 23.5µs |
| 最大 | 418.1µs | 423.4µs |

**→ REST が平均レイテンシで 1.70 倍高速**（ペイロードが極めて小さい場合は、HTTP/1.1 + keep-alive でもオーバーヘッドが小さく、gRPC のフレーミングコストが相対的に目立つケースがある）

### テスト2: 大きいペイロード (100ユーザーリスト)

|  | gRPC | REST |
|---|---|---|
| 経過時間(全体) | 58.42ms | 113.42ms |
| 平均(1件あたり) | 58.4µs | 113.4µs |
| 最小 | 45.7µs | 99.1µs |
| 最大 | 241.6µs | 1.54ms |

**→ gRPC が平均レイテンシで 1.94 倍高速**（ペイロードが大きくなると Protobuf のバイナリ効率が JSON デコードコストを上回る）

### テスト3: 同時並行リクエスト (並行数: 50)

|  | gRPC | REST |
|---|---|---|
| 経過時間(全体) | 8.46ms | 24.45ms |
| 平均(1件あたり) | 406.6µs | 1.12ms |
| 最小 | 89.2µs | 38.8µs |
| 最大 | 1.55ms | 6.43ms |

**→ gRPC が平均レイテンシで 2.75 倍高速**（HTTP/2 の多重化により、1 接続上で 50 並行が効率的にさばかれる）

> 注: 実測値は実行環境・同一プロセス内での実行有無・CPU 負荷等で変動します。上記は参考値であり、各自の環境で再実行して傾向を確認してください。
